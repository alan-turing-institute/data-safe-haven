...
if ((Get-RDWebClientBrokerCert).Thumbprint -eq $certThumbPrint) {
    Write-Host " [o] Webclient broker certificate has the correct thumbprint"
} else {
    Write-Host " [x] Webclient broker certificate has incorrect thumbprint: '$((Get-RDWebClientBrokerCert).Thumbprint)'"
    throw "Webclient broker certificate has incorrect thumbprint"
}
Write-Host "Checking RDGateway certificate..."
if ((Get-RDCertificate -Role RDGateway -ConnectionBroker $rdsFqdn).Thumbprint -eq $certThumbPrint) {
    Write-Host " [o] RDGateway certificate has the correct thumbprint"
} else {
    Write-Host " [x] RDGateway certificate has incorrect thumbprint: '$((Get-RDWebClientBrokerCert).Thumbprint)'"
