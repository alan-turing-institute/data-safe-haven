...
class TestSREIpRanges:
    ...
    def test_vnet_and_subnets(self):
        ...
        assert SREIpRanges.backup == AzureIPv4Range("10.0.1.8", "10.0.1.15")
        assert SREIpRanges.clamav_mirror == AzureIPv4Range("10.0.1.16", "10.0.1.23")
        assert SREIpRanges.data_configuration == AzureIPv4Range(
            "10.0.1.24", "10.0.1.31"
        )
        assert SREIpRanges.data_private == AzureIPv4Range("10.0.1.32", "10.0.1.39")
        assert SREIpRanges.desired_state == AzureIPv4Range("10.0.1.40", "10.0.1.47")
