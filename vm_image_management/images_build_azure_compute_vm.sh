...
print_usage_and_exit() {
    ...
    echo "  -h                           display help"
    echo "  -s subscription              specify subscription for building the VM. (defaults to '${IMAGES_SUBSCRIPTION}')"
...
if [ "$SOURCEIMAGE" = "Ubuntu" ]; then
    ...
    cp ../environment_configs/cloud_init/cloud-init-buildimage-ubuntu.yaml $TMP_CLOUD_CONFIG_YAML
else
...
if [ $(az group exists --name $RESOURCEGROUP_BUILD) != "true" ]; then
    ...
    az group create --name $RESOURCEGROUP_BUILD --location $IMAGES_LOCATION
fi
...
if [ $(az group exists --name $RESOURCEGROUP_NETWORK) != "true" ]; then
    ...
    az group create --name $RESOURCEGROUP_NETWORK --location $IMAGES_LOCATION
fi
...
cd ../environment_configs/package_lists/ && source generate_python_package_specs.sh > ${BUILD_DIR}/python_package_specs.yaml && cd $BUILD_DIR
...
cd ../environment_configs/package_lists/ && source generate_r_package_specs.sh > ${BUILD_DIR}/r_package_specs.yaml && cd $BUILD_DIR
...
echo -e "${BOLD}Provisioning a new VM image in ${BLUE}$RESOURCEGROUP_BUILD${END} ${BOLD}as part of ${BLUE}$IMAGES_SUBSCRIPTION${END}"
