...
If this does not work, please try deleting everything that has been deployed into the `RG_SHM_RG_SHM_<SHM ID>_SRE_<SRE ID>_REMOTE_DESKTOP` resource group for this SRE and {ref}`attempt to run this step again <deploy_sre_microsoft_deploy_remote_desktop>`.
```
