param(
  [Parameter(Position=0, Mandatory = $true, HelpMessage = "Full chain for SSL certificate, including CA intermediate signing certificate, in ASCII *.pem format")]
  ...
  [Parameter(Position=3, Mandatory = $true, HelpMessage = "Fully qualified domain name for RDS broker")]
  [string]$rdsFqdn
)
