param(
  [Parameter(Position=0, Mandatory = $true, HelpMessage = "Enter path to certbot root directory")]
  [string]$certbotDir,
  [Parameter(Position=1, Mandatory = $true, HelpMessage = "Enter DSG ID (usually a number e.g enter '9' for DSG9)")]
  [string]$dsgId,
  [Parameter(Position=2, Mandatory = $true, HelpMessage = "Enter path to CSR file generated on DSG RDS Gateway VM")]
  [string]$csrPath,
  [Parameter(Position=3, Mandatory = $false, HelpMessage = "Flag to do a 'dry run' (i.e. don't actually contact Let's Encrypt servers)")]
  [bool]$dryRun = $false,
  [Parameter(Position=4, Mandatory = $false, HelpMessage = "Flag to do a test run against the Let's Encrypt test servers")]
  [bool]$testRun = $false

)

if(-not (Test-Path -Path $csrPath)) { throw [System.IO.FileNotFoundException] "$csrPath not found."}
...
$certbotCmd += " --preferred-challenges 'dns' --manual"
