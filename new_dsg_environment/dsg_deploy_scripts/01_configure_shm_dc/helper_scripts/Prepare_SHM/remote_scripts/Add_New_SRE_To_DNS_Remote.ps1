...
Write-Output " - Adding reverse lookup record for SRE Identity subnet (CIDR: $identitySubnetCidr)"
Add-DnsServerPrimaryZone -DynamicUpdate Secure -NetworkId $identitySubnetCidr -ReplicationScope Domain
Write-Output " - Adding reverse lookup record for SRE RDS subnet (CIDR: $rdsSubnetCidr)"
Add-DnsServerPrimaryZone -DynamicUpdate Secure -NetworkId $rdsSubnetCidr -ReplicationScope Domain
Write-Output " - Adding reverse lookup record for SRE Data subnet (CIDR: $dataSubnetCidr)"
Add-DnsServerPrimaryZone -DynamicUpdate Secure -NetworkId $dataSubnetCidr -ReplicationScope Domain

#Create conditional forwarders
Write-Output " - Adding conditional forwarder record for SRE domain (domain: $dsgFqdn; SRE DC IP: $dsgDcIp)"
