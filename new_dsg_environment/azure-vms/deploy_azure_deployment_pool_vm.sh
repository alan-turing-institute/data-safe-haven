...
if [ "$(az network nsg show --resource-group $RESOURCEGROUP --name $NSG_NAME 2> /dev/null)" = "" ]; then
    ...
    az network nsg rule create --resource-group $RESOURCEGROUP --nsg-name $NSG_NAME --direction Inbound --priority 1000 --name AllowMosh --description "Allow Mosh" --access "Allow" --source-address-prefixes "*" --source-port-ranges "*" --destination-address-prefixes 193.60.220.253 193.60.220.240 --destination-port-ranges "60000-61000" --protocol "UDP"
    az network nsg rule create --resource-group $RESOURCEGROUP --nsg-name $NSG_NAME --direction Inbound --priority 1500 --name AllowEternalTerminal --description "Allow Eternal Terminal" --access "Allow" --source-address-prefixes "*" --source-port-ranges "*" --destination-address-prefixes 193.60.220.253 193.60.220.240 --destination-port-ranges "2022" --protocol "TCP"
