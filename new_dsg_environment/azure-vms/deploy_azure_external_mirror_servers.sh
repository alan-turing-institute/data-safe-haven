...
if [ "$(az network nsg show --resource-group $RESOURCEGROUP --name $NSG_EXTERNAL 2> /dev/null)" = "" ]; then
    ...
    az network nsg rule create --resource-group $RESOURCEGROUP --nsg-name $NSG_EXTERNAL --direction Inbound --name updateInbound --description "Allow ports 80, 443 and 8080 for updating mirrors" --source-address-prefixes Internet --destination-port-ranges 80 443 8080 --protocol TCP --destination-address-prefixes $IP_RANGE_EXTERNAL --priority 300
    ...
    az network nsg rule create --resource-group $RESOURCEGROUP --nsg-name $NSG_EXTERNAL --direction Outbound --name updateOutbound --description "Allow ports 80, 443 and 8080 for updating mirrors" --source-address-prefixes $IP_RANGE_EXTERNAL --destination-port-ranges 80 443 8080 --protocol TCP --destination-address-prefixes Internet --priority 300
