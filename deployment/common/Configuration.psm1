...
function Add-SreConfig {
    ...
    $config.sre.rds = [ordered]@{
        ...
        sessionHost3 = [ordered]@{
            adminPasswordSecretName = "$($config.sre.shortName)-vm-admin-password-rds-sh3"
            vmName = "REV-SRE-$($config.sre.id)".ToUpper() | Limit-StringLength 15
            vmSize = "Standard_DS2_v2"
            ip = Get-NextAvailableIpInRange -IpRangeCidr $config.sre.network.vnet.subnets.rds.cidr -Offset 7
            nsg = "NSG_SRE_$($config.sre.id)_RDS_SESSION_HOSTS".ToUpper()
            disks = [ordered]@{
                os = [ordered]@{
                    sizeGb = "128"
