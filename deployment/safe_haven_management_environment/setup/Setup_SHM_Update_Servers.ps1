...
# ------------------------------------------
$null = Deploy-ResourceGroup -Name $config.updateServers.rg -Location $config.location


# Deploy the Linux update server
# ------------------------------
$cloudInitYaml = Expand-MustacheTemplate -TemplatePath (Join-Path $PSScriptRoot ".." "cloud_init" "cloud-init-update-server-apt.mustache.yaml") -Parameters $config
...
$vmName = $config.updateServers.linux.vmName
$params = @{
    AdminPassword          = (Resolve-KeyVaultSecret -VaultName $config.keyVault.name -SecretName $config.updateServers.linux.adminPasswordSecretName -DefaultLength 20)
