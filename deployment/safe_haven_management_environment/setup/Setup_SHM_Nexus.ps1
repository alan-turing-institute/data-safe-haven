...
if (Get-AzVirtualNetworkPeering -Name "PEER_$($config.network.vnet.name)" -VirtualNetworkName $config.network.repositoryVnet.name -ResourceGroupName $config.network.vnet.rg) {
    Remove-AzVirtualNetworkPeering -Name "PEER_$($config.network.vnet.name)" -VirtualNetworkName $config.network.repositoryVnet.name -ResourceGroupName $config.network.vnet.rg -Force
}
if (Get-AzVirtualNetworkPeering -Name "PEER_$($config.network.repositoryVnet.name)" -VirtualNetworkName $config.network.vnet.name -ResourceGroupName $config.network.vnet.rg) {
    Remove-AzVirtualNetworkPeering -Name "PEER_$($config.network.repositoryVnet.name)" -VirtualNetworkName $config.network.vnet.name -ResourceGroupName $config.network.vnet.rg -Force
}
