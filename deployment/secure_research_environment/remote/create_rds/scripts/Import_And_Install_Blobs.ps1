...
# Download artifacts
Write-Host "Downloading $($remoteFilePaths.Count) files to '$downloadDir'"
foreach ($remoteFilePath in $remoteFilePaths) {
    ...
    if (-Not (Test-Path -Path $localDir)) {
        $_ = New-Item -ItemType directory -Path $localDir
    }
    ...
    $_ = Invoke-WebRequest -Uri "${remoteUrl}${sasToken}" -OutFile $localFilePath
    if ($?) {
